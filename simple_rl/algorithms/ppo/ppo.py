...
from simple_rl.modules.modules import BasePolicy, BaseValue
...
class PPO:
    def __init__(
        ...
        self.policy.to(self.env_spec.device)
        self.value.to(self.env_spec.device)
    ...
    def load(self, path:str) :
        ...
        self.value.load_state_dict(state_dict['value_state_dict'])
    ...
    def act(self, obs_raw:th.Tensor) :
        obs = self.preprocess(obs_raw)
        with th.no_grad():
            self._last_act_pdf = self.policy.compute(obs)
            return self._last_act_pdf.sample()
    ...
    def update(self, train_rate:float) -> Dict[str,float]:
        ...
        with th.no_grad() :
            ...
            next_value = self.value.compute(next_obs)
        ...
        for minibatchs in self.buffer.get_minibatch_generator() :
            ...
            pdf_old = self.policy.get_pdf_cls()(*pdf_old)

            if self.cfg.normalize_advantage :
                advantage = (advantage - advantage.mean()) / (advantage.std() + 1e-8)
            
            pdf = self.policy.compute(obs)
            # adaptive learning rate
            if self.cfg.desired_kl is not None :
                with th.no_grad() :
                    policy_kld = self.policy.get_pdf_cls().kld(pdf_old, pdf).mean().item()
            ...
            if self.cfg.grad_norm_clip is not None :
                th.nn.utils.clip_grad_norm_(list(self.policy.parameters())+list(self.value.parameters()), self.cfg.grad_norm_clip)
            self.optimizer.step()
            # logging
            if self.cfg.desired_kl is not None :
